    ...
    >
      {props.components.map((component, index) => {
        return (
          ...
          >
            <component.icon />
            <span className={`self-center`}>{component.text}</span>
          </button>
        );
      })}
    </div>
...
const Modal = props => {
  ...
  return (
    <Fragment>
      <div
        className={`font-body bg-red-400 px-4 py-3 pt-5 text-white w-60 rounded-md flex flex-col hover:border-slate-400 border-2 outline-dotted transition-all`}
      >
        <div className={classes.modalHeading}>
          <h2>{props.title}</h2>
          <hr />
          <p className={`p-2 text-gray-100/50`}>
            ...
            <button>
              ...
              <Dropdown
                className={`
                            ...
                            ${
                              show
                                ? "h-auto opacity-100 py-3 pointer-events-auto cursor-pointer"

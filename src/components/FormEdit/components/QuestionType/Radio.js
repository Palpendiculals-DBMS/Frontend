...
import RadioInput from '../Basic/Radio';
      ...
      >
        ...
        <div className={`flex flex-col py-4`}>
            ...
            QuestionState.options.map((option, index) => {
              return (
                <>
                  <RadioInput
                    key={index}
                    className={`form-check-input appearance-none rounded-full h-3 w-3 border border-gray-300 bg-white checked:bg-red-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer`}
          ...
          <div className={`flex my-4`}>
            <button
              className={`mx-2 ml-4 text-lg `}
              onClick={addNewOption}
            >
              <BsFillPlusSquareFill className={`text-red-500 shadow-lg shadow-red-500/30 hover:text-red-800 transition-all`} />
            </button>
            <div className={`font-body opacity-50`}>
              Add New
            </div>
          </div>
